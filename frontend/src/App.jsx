...
const socketUrl =
  import.meta.env.MODE === "production" ? "" : `http://${devDomain}:3001`;
...
const renderMarkdown = (text) => {
  ...
  const unwrapped =
    ...
    trimmed.indexOf("<p>", 1) === -1
      ...
      : trimmed;
  // Highlight @mentions
  const withMentions = DOMPurify.sanitize(unwrapped, {
    ADD_ATTR: ["target"],
  }).replace(
    /@([\w][\w\s]*?)(?=[\s,.:;!?<]|$)/g,
...
const processAllMarkdownInNode = (el) => {
  ...
  while (changed && iterations < 50) {
    ...
    while ((node = walker.nextNode())) {
      ...
      if (
        node.parentNode !== el &&
        ["STRONG", "B", "EM", "I", "DEL", "S", "CODE"].includes(
    ...
    <div className="cropper-overlay">
      <div className="cropper-container">
        ...
        <div className="cropper-controls">
          <label
            style={{ fontSize: "0.9rem", color: "var(--md-sys-color-outline)" }}
          >
            Zoom
          </label>
          <input
          ...
          <div className="cropper-btns">
            ...
            <button
              onClick={async () =>
                onComplete(await getCroppedImg(image, croppedAreaPixels))
              }
...
const LOCATION_TZ_MAP = [
  ...
  [
    /toronto|montreal|ottawa|quebec|ontario|vancouver|calgary|edmonton|winnipeg/i,
    "America/Toronto",
  ],
  [
  ...
  [
    /berlin|munich|hamburg|frankfurt|cologne|germany|deutschland/i,
    "Europe/Berlin",
  ],
  ...
  [
    /stockholm|sweden|oslo|norway|copenhagen|denmark|helsinki|finland/i,
    "Europe/Stockholm",
  ],
...
const FontPicker = ({ value, onApply }) => {
  ...
  const popularMatches = popularFonts.filter((f) =>
    f.toLowerCase().includes(query.toLowerCase()),
  );
  const otherMatches = googleFonts.filter(
    (f) =>
      !popularFonts.includes(f) &&
      f.toLowerCase().includes(query.toLowerCase()),
  );
...
const AdminPanel = ({
  ...
  const handleSuspendUser = async (userId) => {
    try {
      const res = await fetch(
        `${socketUrl}/api/admin/users/${userId}/suspend`,
        {
          method: "PUT",
      ...
      if (res.ok) {
        ...
        setUsers((prev) =>
          prev.map((u) =>
            u.id === userId ? { ...u, suspended: data.suspended } : u,
          ),
        );
      }
    } catch (e) {
  ...
  const handleBulkSuspend = async (suspend) => {
    ...
    try {
      ...
      if (res.ok) {
        setUsers((prev) =>
          prev.map((u) =>
            selectedUsers.has(u.id) ? { ...u, suspended: suspend ? 1 : 0 } : u,
          ),
        );
  ...
  const handleBulkDelete = async () => {
    if (selectedUsers.size === 0) return;
    if (
      !confirm(`Delete ${selectedUsers.size} user(s)? This cannot be undone.`)
    )
      return;
    try {
  ...
  const handleExportUsers = async (format = "csv") => {
    try {
      const res = await fetch(
        `${socketUrl}/api/admin/users/export?format=${format}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const fetchReports = async (status = "pending") => {
    try {
      const res = await fetch(
        `${socketUrl}/api/admin/reports?status=${status}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const handleRevokeApiKey = async (id) => {
    if (
      !confirm("Revoke this API key? Applications using it will lose access.")
    )
      return;
    try {
  ...
  const fetchEnvironment = async () => {
    try {
      const res = await fetch(`${socketUrl}/api/admin/environment`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleAdminResetPassword = async () => {
    ...
    try {
      const res = await fetch(
        `${socketUrl}/api/admin/users/${editingUser.id}/password`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      setAdminResetMsg({
        ok: res.ok,
        text: res.ok ? "Password reset successfully" : data.error,
      });
      ...
      <div style={{ maxWidth: "min(95vw, 1200px)", margin: "0 auto" }}>
        ...
        {activeTab === "dashboard" &&
          ...
          (() => {
            ...
            const maxVol = Math.max(
              ...(dashboardStats.messageVolume?.map((v) => v.count) || [1]),
              1,
            );
            const parseUA = (ua) => {
              ...
              <div>
                ...
                <div className="dash-grid">
                  ...
                  <div className="dash-card">
                    ...
                    <div>
                      <div className="dash-card-value">
                        {dashboardStats.activeUsers24h}
                      </div>
                      <div className="dash-card-label">
                        Active (24h) / {dashboardStats.activeUsers7d} (7d)
                      </div>
                    </div>
                  </div>
                  <div className="dash-card">
                    <div
                      ...
                      style={{
                        background:
                          "color-mix(in srgb, #2196F3 15%, transparent)",
                      }}
                    >
                      <svg
                        width="20"
                  ...
                  <div className="dash-card">
                    ...
                    <div>
                      <div className="dash-card-value">
                        {fmt(dashboardStats.storageUsedBytes)}
                      </div>
                      ...
                      <div style={{ position: "relative", height: "160px" }}>
                        ...
                        >
                          {dashboardStats.messageVolume.map((v, i) => {
                            ...
                            const x =
                              i * (100 / dashboardStats.messageVolume.length);
                            const w =
                              (100 / dashboardStats.messageVolume.length) * 0.7;
                            return (
                              <g key={i}>
                                <rect
                                  x={`${x}%`}
                        ...
                        >
                          <span>
                            {dashboardStats.messageVolume[0]?.date?.slice(5)}
                          </span>
                          <span>
                            {dashboardStats.messageVolume[
                              dashboardStats.messageVolume.length - 1
                            ]?.date?.slice(5)}
                          </span>
                        </div>
                      </div>
                              ...
                              >
                                ...
                                >
                                  <button
                                    onClick={async () => {
                                      if (
                                        !confirm(`Disconnect ${displayName}?`)
                                      )
                                        return;
                                      try {
                                        await fetch(
                                          `${socketUrl}/api/admin/disconnect`,
                                          {
                                            method: "POST",
                                            headers: {
                                              "Content-Type":
                ...
                <div className="dash-section">
                  ...
                  >
                    {[
                      {
                        label: "Uptime",
                        value: uptimeFmt(dashboardStats.serverUptime),
                      },
                      ...
                      {
                        label: "Database",
                        value: fmt(dashboardStats.dbSizeBytes),
                      },
                      {
                        label: "Memory (Heap)",
              ...
              {(() => {
                ...
                return [...users]
                  ...
                  .map((u) => {
                    ...
                    const onboardScore = [hasName, hasAvatar, hasEmail].filter(
                      Boolean,
                    ).length;
                ...
                <div className="admin-edit-col">
                  <div className="ae-section">
                    ...
                    >
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              if (file) {
                                ...
                                reader.onload = (event) =>
                                  setAdminCroppingImage(event.target.result);
                      ...
                      {editingUser.avatar && (
                        <button
                          type="button"
                          onClick={() =>
                            setEditingUser({ ...editingUser, avatar: null })
                          }
                          style={{
                            background: "none",
                            border: "none",
                    ...
                    <div
                      style={{
                        position: "relative",
                        zIndex: showLocSuggestions ? 10 : 1,
                      }}
                    >
                      ...
                      <input
                        ...
                        onFocus={() => {
                          if (editingUser.location?.length >= 2)
                            setShowLocSuggestions(true);
                        }}
                        onBlur={() =>
                      ...
                      {showLocSuggestions && locSuggestions.length > 0 && (
                        <div
                          ...
                          style={{ top: "calc(100% + 4px)", padding: "4px" }}
                        >
                  ...
                  <div className="ae-section">
                    ...
                    >
                      {STATUS_ICONS.map((icon) => (
                        <button
                          ...
                          onClick={() => {
                            const ne =
                              icon.id === "none" ? "available" : icon.id;
                            setEditingUser({
                              ...editingUser,
                              status_emoji: ne,
                              status_text:
                    ...
                    {editingUser.status_emoji &&
                      editingUser.status_emoji !== "none" && (
                        <div
                          style={{
                            display: "flex",
                            alignItems: "center",
                        ...
                        >
                          <StatusIcon
                            statusId={editingUser.status_emoji}
                            size={18}
                          />
                          <input
                            className="ae-input"
                            ...
                            placeholder={
                              STATUS_ICONS.find(
                                (s) => s.id === editingUser.status_emoji,
                  ...
                  <div className="ae-section">
                    ...
                    <div>
                      ...
                      <FontPicker
                        ...
                        onApply={(val) =>
                          setEditingUser({ ...editingUser, font_family: val })
                        }
                      />
                    </div>
                  </div>
              ...
              {assets
                ...
                .map((a) => {
                  ...
                  const isVideo =
                    ...
                    !a.file.includes("voice-");
                  const isAudio =
                    (a.type && a.type.startsWith("audio/")) ||
                    a.file.includes("voice-");
                  const sizeStr =
                    a.size >= 1024
                      ? `${(a.size / 1024).toFixed(1)} MB`
                      : `${a.size} KB`;
                  return (
                    <div
                      key={a.file}
          ...
          <div className="dash-section" style={{ marginBottom: "0.85rem" }}>
            ...
            >
              ...
              >
                ...
                {[...new Set(spaces.map((s) => s.created_by))].map(
                  (creator) => {
              ...
              {[...spaces]
                ...
                .map((s) => {
                  ...
                  if (isSelf) {
                    ...
                    friendlyName = su
                      ? `Notes to Self (${su.first_name || selfUser})`
                      : `Notes to Self (${selfUser})`;
                  } else if (isDMConvo) {
        ...
        {userToDelete && (
          ...
          >
            ...
            >
              <h2 style={{ color: "var(--md-sys-color-error)" }}>
                Delete User?
              </h2>
              <p>
        ...
        {assetToDelete && (
          ...
          >
            ...
            >
              ...
              <p>
                Are you sure you want to delete{" "}
                <strong>{assetToDelete.file}</strong>?
              </p>
              <div
                style={{ display: "flex", gap: "1rem", marginTop: "1.5rem" }}
              >
        ...
        {spaceToDeleteAdmin && (
          ...
          >
            ...
            >
              ...
              <p>
                Are you sure you want to delete{" "}
                <strong>#{spaceToDeleteAdmin.name}</strong>? All messages will
                be lost.
              </p>
              <div
                style={{ display: "flex", gap: "1rem", marginTop: "1.5rem" }}
                  ...
                  >
                    <button
                      onClick={() =>
                        updateConfig(
                          configKey,
                          serverConfig[configKey] === "true" ? "false" : "true",
                        )
                      }
                      style={{
                        position: "relative",
                    ...
                    >
                      <div
                        style={{
                          ...
                          top: "2px",
                          left:
                            serverConfig[configKey] === "true" ? "22px" : "2px",
                          width: "20px",
                          height: "20px",
                          borderRadius: "50%",
                          background: "white",
                        ...
                        ref={(el) => {
                          if (el && !el._prado) {
                            ...
                            el.addEventListener("input", () => {
                              const span = el
                                .closest("[data-color-row]")
                                ?.querySelector("[data-color-hex]");
                              if (span) span.textContent = el.value;
                            });
                            el.addEventListener("change", () =>
                              updateConfig(configKey, el.value),
                            );
                          }
                        }}
                        style={{
              ...
              <div>
                ...
                <div className="dash-two-col-equal">
                  ...
                  <div className="dash-section">
                    ...
                    {inputRow(
                      ...
                      "select",
                      {
                        options: [
                          {
                            value: "open",
                            label: "Open (anyone can register)",
                          },
                          {
                            value: "closed",
                            label: "Closed (no new registrations)",
                          },
                        ],
                      },
                    )}
                    ...
                    {inputRow(
                      ...
                      "text",
                      {
                        ...
                        description:
                          "Comma-separated list of allowed email domains. Leave empty for no restriction.",
                      },
                    )}
                  </div>
                  ...
                  <div className="dash-section">
                    ...
                    {inputRow("App Name", "app_name", "text", {
                      placeholder: "Prado Chat",
                    })}
                    {/* Custom Logo Upload */}
                    <div
                      style={{
                          ...
                          <input
                            ...
                            onChange={async (e) => {
                              ...
                              try {
                                ...
                                const data = await res.json();
                                if (data.logo)
                                  setAppLogo(data.logo + "?t=" + Date.now());
                              } catch (err) {
                ...
                >
                  ...
                  <div className="dash-section">
                    ...
                    {inputRow("From Address", "email_from", "text", {
                  ...
                  <div className="dash-section">
                    ...
                    {inputRow("TURN Server URL", "turn_server", "text", {
                      ...
                      description:
                        "TURN relay for NAT traversal in video calls",
                    })}
                    {inputRow("TURN Username", "turn_username", "text", {
                    ...
                    {inputRow("STUN Server URL", "stun_server", "text", {
                      ...
                      description:
                        "STUN server for connection discovery. Defaults to Google's.",
                    })}
                            ...
                            try {
                              ...
                              await new Promise((resolve) => {
                                ...
                                pc2.onicecandidate = (e) => {
                                  if (e.candidate && e.candidate.candidate) {
                                    const typ =
                                      e.candidate.candidate.match(/typ (\w+)/);
                                    if (typ && typ[1] === "relay") {
                                      result.relay = true;
                                      result.candidates.push({
                                        type: "relay (isolated)",
                        ...
                        >
                          ...
                          {turnTestStatus.candidates?.length > 0 && (
                            <details style={{ marginTop: "4px" }}>
                              ...
                              >
                                {turnTestStatus.candidates.length} candidate(s)
                                discovered
                              </summary>
                              <div
                                style={{
                  ...
                  <div className="dash-section">
                    ...
                    {inputRow("Giphy API Key", "giphy_api_key", "text", {
                      ...
                      description:
                        "API key for GIF search. Get one at developers.giphy.com",
                    })}
                  </div>
                </div>
                    ...
                    >
                      ...
                      {envInfo.env &&
                        Object.entries(envInfo.env).map(([key, entry]) => {
                          ...
                          const status =
                            typeof entry === "object" ? entry.status : "ok";
                          const statusColor =
                            status === "error"
                              ? "#ef4444"
                              : status === "warn"
                          ...
                          >
                            ...
                            {r.status === "pending" && (
                              <>
                                <button
                                  onClick={() =>
                                    handleReportAction(r.id, "resolved", true)
                                  }
                                  style={{
                                    padding: "4px 10px",
                                    borderRadius: "6px",
                                ...
                                <button
                                  onClick={() =>
                                    handleReportAction(r.id, "dismissed")
                                  }
                                  style={{
                                    padding: "4px 10px",
                                    borderRadius: "6px",
                      ...
                      {auditLogs?.logs?.map((log) => (
                        ...
                        >
                          <span style={{ fontSize: "1rem", flexShrink: 0 }}>
                            {
                              (actionLabels[log.action] || log.action).split(
                                " ",
                              )[0]
                            }
                          </span>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            >
                              {" "}
                              {actionLabels[log.action]?.substring(2) ||
                                log.action}{" "}
                            </span>
                            {log.target_id && (
                              <span
        ...
        {activeTab === "broadcast" && (
          <div className="dash-section">
            ...
            >
              Send a real-time announcement banner to all connected users. The
              message appears at the top of their screen with an auto-dismiss
              countdown.
            </p>
            <textarea
              value={broadcastText}
...
function App() {
  ...
  const [inlineReplyThreadId, setInlineReplyThreadId] = useState(null);
  const [inlineReplyText, setInlineReplyText] = useState("");
  const [threadToast, setThreadToast] = useState(null); // { senderName, parentMsgId, replyMsgId }
  const [threadUnreads, setThreadUnreads] = useState({}); // { [rootMsgId]: count }
  const [newMsgPill, setNewMsgPill] = useState(null); // { senderName, count, lastText }
  ...
  const [uiScale, setUiScale] = useState(
    () => parseFloat(localStorage.getItem("uiScale")) || 1.0,
  );
  ...
  const collapsedThreadsRef = useRef({});
  ...
  useEffect(() => {
    collapsedThreadsRef.current = collapsedThreads;
  }, [collapsedThreads]);
  useEffect(() => {
  ...
  const handleSpaceSelect = async (space) => {
    if (space.id === currentSpace?.id) {
      // Already active — just close sidebar on mobile
    ...
    if (
      ...
      try {
        ...
        if (keysRes.ok) {
          ...
          if (targetKeyObj) {
            const roomKey = await decryptRoomKeyWithPrivateKey(
              targetKeyObj.encrypted_room_key,
              privateKeyRef.current,
            );
            ...
            try {
              const escrowRes = await fetch(
                `${socketUrl}/api/spaces/${space.id}/request-key`,
                {
                  headers: { Authorization: `Bearer ${token}` },
                },
              );
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (
        ...
        !startChatRef.current.contains(event.target)
      ) {
        setShowStartChatMenu(false);
      }
      if (
        spaceMenuRef.current &&
        !spaceMenuRef.current.contains(event.target)
      ) {
  ...
  const isNearBottom = () => {
    if (!chatEndRef.current || !chatEndRef.current.parentElement) return true;
    const container = chatEndRef.current.parentElement;
  ...
  // Auto-dismiss new message pill when user scrolls to bottom
  useEffect(() => {
    const container = chatEndRef.current?.parentElement;
    if (!container) return;
    const handleScroll = () => {
  ...
  useEffect(() => {
    if (spaces.length > 0 && !spaces.find((s) => s.id === currentSpace.id)) {
      const selfDm = spaces.find(
        (s) => s.is_dm === 1 && s.name.startsWith("self_"),
      );
    ...
    const fetchSpaces = async () => {
      try {
        const [spacesRes, keysRes] = await Promise.all([
          fetch(`${socketUrl}/api/spaces`, {
            headers: { Authorization: `Bearer ${token}` },
            cache: "no-store",
          }),
          fetch(`${socketUrl}/api/spaces/keys`, {
            headers: { Authorization: `Bearer ${token}` },
            cache: "no-store",
          }),
        ]);
        ...
        if (spacesRes.ok) {
              ...
              await Promise.all(
                keysData.map(async (kd) => {
                  try {
                    ...
                    console.error(
                      `Failed to silently decrypt room key for space ${kd.space_id}`,
                      e,
                    );
                  }
                }),
          ...
          if (pKey) {
            const missingSpaces = spacesData.filter(
              (s) => isEncryptedSpace(s) && !newActiveKeys[s.id],
            );
                  ...
                  try {
                    ...
                    if (keyRes.ok) {
                      ...
                      if (encrypted_room_key) {
                        ...
                        await syncRoomKeyToIDB(space.id, roomKey);
                        console.log(
                          `[E2EE] Recovered key from escrow for space ${space.id}`,
                        );
                      }
                    }
                  } catch (e) {
                    console.error(
                      `[E2EE] Escrow recovery failed for space ${space.id}`,
                      e,
                    );
                  }
                }),
          ...
          if (spacesData.length > 0) {
            ...
            const lastSpace = lastSpaceId
              ? spacesData.find((s) => Number(s.id) === Number(lastSpaceId))
              : null;
            const selfDm = spacesData.find(
              (s) => s.is_dm === 1 && s.name.startsWith("self_"),
            );
            ...
            setTimeout(() => {
              spacesData.forEach(async (space) => {
                ...
                try {
                  const res = await fetch(
                    `${socketUrl}/api/messages/${space.id}`,
                    {
                      headers: { Authorization: `Bearer ${token}` },
                    },
                  );
  ...
  useEffect(() => {
    ...
    if (
      !selfDm ||
      activeKeys[selfDm.id] ||
      provisioningLocksRef.current.has(selfDm.id)
    )
      return;
    ...
    const provisionKey = async () => {
      try {
        ...
        const peerEnc = await encryptRoomKeyWithPublicKey(
          roomKey,
          profileData.public_key,
        );
        const res = await fetch(`${socketUrl}/api/spaces/${selfDm.id}/invite`, {
          ...
          body: JSON.stringify({
            invited_users: [profileData.id],
            keyShares: { [profileData.id]: peerEnc },
          }),
        });
        if (res.ok) {
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      if (didDecrypt) setMessages(newMsgs);
  ...
  useEffect(() => {
    ...
    newSocket.on("connect", () => {
      ...
      fetch(`${socketUrl}/api/ice-servers`, {
        headers: { Authorization: `Bearer ${token}` },
      })
        .then((r) => r.json())
        .then((data) => {
    ...
    newSocket.on(
      "user typing",
      ({ username: typer, spaceId, avatar, first_name }) => {
        setTypingUsers((prev) => {
          if (
            prev.find(
              (u) =>
                u.username === typer && Number(u.spaceId) === Number(spaceId),
            )
          )
            return prev;
          return [
            ...prev.filter((u) => u.username !== typer),
            { username: typer, spaceId, avatar, first_name },
          ];
        });
      },
    ...
    newSocket.on(
      "call-ringing",
      ({ spaceId: ringSpaceId, caller, audioOnly }) => {
          ...
          ringCountdownRef.current = setInterval(() => {
            ...
            if (count <= 0) {
              ...
              setIncomingCall((prev) =>
                prev?.spaceId === ringSpaceId ? null : prev,
              );
            }
          }, 1000);
        }
    ...
    newSocket.on("space history", async (history) => {
      ...
      processedHistory = await Promise.all(
        history.map(async (msg) => {
          if (!msg.text || typeof msg.text !== "string") return msg;
          const d = await tryDecryptMsg(
            msg.text,
            activeKeysRef.current[spaceId],
          );
          return { ...msg, ...d };
        }),
      );
    ...
    newSocket.on("chat message", async (msg) => {
      ...
      if (Number(currentSpaceRef.current) === spaceId) {
        ...
        // ── New message pill (Webex-style) — show when scrolled up ──
        if (processedMsg.sender !== currentUsername && !isNearBottom()) {
          const senderName = processedMsg.first_name
            ? `${processedMsg.first_name} ${processedMsg.last_name || ""}`.trim()
            : processedMsg.sender;
          const preview = processedMsg.asset
            ? "📎 Attachment"
            : (processedMsg.text || "").substring(0, 60);
          setNewMsgPill((prev) => ({
            senderName,
        ...
        if (processedMsg.reply_to && processedMsg.sender !== currentUsername) {
          // Walk the full reply chain to find the true root message
          const currentMsgs = messagesRef.current;
          let rootId = processedMsg.reply_to;
          const visited = new Set();
          while (true) {
            visited.add(rootId);
            const parent = currentMsgs.find((m) => m.id === rootId);
            if (parent && parent.reply_to && !visited.has(parent.reply_to)) {
              rootId = parent.reply_to;
            } else {
              break;
            }
          }
          const rootMsg = currentMsgs.find((m) => m.id === rootId);

          // Collect ALL message IDs in this thread (root + all nested replies)
          const threadMsgIds = new Set([rootId]);
          let prevSize = 0;
          while (threadMsgIds.size > prevSize) {
            prevSize = threadMsgIds.size;
            currentMsgs.forEach((m) => {
              if (m.reply_to && threadMsgIds.has(m.reply_to)) {
                threadMsgIds.add(m.id);
              }
            });
          }

          // Toast: show when someone replies to a thread you participate in
          const isRootOwner = rootMsg && rootMsg.sender === currentUsername;
          const hasRepliedInThread = currentMsgs.some(
            (m) => threadMsgIds.has(m.id) && m.sender === currentUsername,
          );
          if (isRootOwner || hasRepliedInThread) {
            const senderName = processedMsg.first_name
              ? `${processedMsg.first_name} ${processedMsg.last_name || ""}`.trim()
              : "Someone";
            setThreadToast({
              senderName,
              parentMsgId: rootId,
              replyMsgId: processedMsg.id,
            });
            // Auto-dismiss after 5 seconds
            setTimeout(() => setThreadToast(null), 5000);

            // Auto-expand the thread if it was collapsed
            ...
            // Play a soft thread-reply chime (2 gentle notes)
            try {
              const ctx = new (
                window.AudioContext || window.webkitAudioContext
              )();
              ...
              playNote(392, ctx.currentTime, 0.12); // G4 — softer than mention chime
              playNote(523.25, ctx.currentTime + 0.12, 0.18); // C5
          ...
          // Thread unread badge: increment if the thread is collapsed
          if (collapsedThreadsRef.current[rootId]) {
            setThreadUnreads((prev) => ({
              ...prev,
        ...
        // ── Cross-space thread toast ──
        if (processedMsg.reply_to && processedMsg.sender !== currentUsername) {
          const cachedMsgs = messageCacheRef.current[spaceId];
          if (cachedMsgs && cachedMsgs.length > 0) {
            // Walk the full reply chain to find the true root
            let rootId = processedMsg.reply_to;
            const visited = new Set();
            while (true) {
              visited.add(rootId);
              const parent = cachedMsgs.find((m) => m.id === rootId);
              if (parent && parent.reply_to && !visited.has(parent.reply_to)) {
                rootId = parent.reply_to;
              } else {
                break;
            ...
            const rootMsg = cachedMsgs.find((m) => m.id === rootId);

            // Collect all thread message IDs
            const threadMsgIds = new Set([rootId]);
            let prevSize = 0;
            while (threadMsgIds.size > prevSize) {
              prevSize = threadMsgIds.size;
            ...
            const isRootOwner = rootMsg && rootMsg.sender === currentUsername;
            const hasRepliedInThread = cachedMsgs.some(
              (m) => threadMsgIds.has(m.id) && m.sender === currentUsername,
            );
            if (isRootOwner || hasRepliedInThread) {
              const senderName = processedMsg.first_name
                ? `${processedMsg.first_name} ${processedMsg.last_name || ""}`.trim()
                : "Someone";
              const targetSpace = spacesRef.current?.find(
                (s) => s.id === spaceId,
              );
              const crossName =
                targetSpace?.is_dm === 1 ? null : targetSpace?.name;
              setThreadToast({
                senderName,
                parentMsgId: rootId,
                replyMsgId: processedMsg.id,
                crossSpaceId: spaceId,
                crossSpaceName: crossName || null,
                crossIsDm: targetSpace?.is_dm === 1,
              });
              setTimeout(() => setThreadToast(null), 5000);

              // Play the thread chime
              try {
                const ctx = new (
                  window.AudioContext || window.webkitAudioContext
                )();
                ...
                playNote(392, ctx.currentTime, 0.12);
                playNote(523.25, ctx.currentTime + 0.12, 0.18);
                setTimeout(() => ctx.close().catch(() => {}), 800);
              } catch (e) {
                /* AudioContext not available */
              }
      ...
      if (text && typeof text === "string") {
        ...
        if (d.raw_text) {
          ...
          setMessages((prev) =>
            prev.map((m) =>
              m.id === id
                ? { ...m, text: d.text, raw_text: d.raw_text, edited }
                : m,
            ),
          );
          return;
        ...
        setMessages((prev) =>
          prev.map((m) => (m.id === id ? { ...m, text: d.text, edited } : m)),
        );
      } else {
        setMessages((prev) =>
    ...
    newSocket.on(
      ...
      ({
        ...
        setMessages((prev) =>
          prev.map((m) =>
            m.sender === uName ? { ...m, avatar, font_family } : m,
          ),
        );
        setSpaces((prev) =>
          prev.map((s) =>
            s.is_dm === 1 && s.dm_username === uName
              ? { ...s, dm_avatar: avatar }
              : s,
          ),
        );
        setCurrentSpace((prev) =>
          prev && prev.is_dm === 1 && prev.dm_username === uName
            ? { ...prev, dm_avatar: avatar }
            : prev,
        );
        setAllUsers((prev) =>
          prev.map((u) =>
    ...
    newSocket.on("request_room_key", async (data) => {
      const { spaceId, requesterId, requesterPublicKey, requesterSocketId } =
        data;
      const myKey = activeKeysRef.current[spaceId];
      if (!myKey || !requesterPublicKey) return;
      try {
        const encryptedForRequester = await encryptRoomKeyWithPublicKey(
          myKey,
          requesterPublicKey,
        );
    ...
    newSocket.on("rekey_space", async (data) => {
      ...
      try {
        ...
        const membersRes = await fetch(
          `${socketUrl}/api/spaces/${spaceId}/members`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
        ...
        for (const m of membersWithKeys) {
          try {
            keyShares[m.id] = await encryptRoomKeyWithPublicKey(
              newRoomKey,
              m.public_key,
            );
          } catch (e) {
        ...
        await fetch(`${socketUrl}/api/spaces/${spaceId}/invite`, {
          ...
          body: JSON.stringify({
            invited_users: Object.keys(keyShares).map(Number),
            keyShares,
          }),
        });
        ...
        console.log(
          `[E2EE] Re-keyed space ${spaceId} with ${membersWithKeys.length} members`,
        );
      } catch (e) {
    ...
    newSocket.on("space created", (newSpace) => {
      ...
      if (newSpace.is_dm === 1) reloadSpacesSilently();
        ...
        setSpaces((prev) => {
          if (!prev.find((s) => s.id === newSpace.id))
    ...
    newSocket.on("space invited", (spaceObj) => {
      ...
      if (spaceObj.is_dm === 1) reloadSpacesSilently();
        ...
        setSpaces((prev) => {
          if (!prev.find((s) => s.id === spaceObj.id))
            return [...prev, spaceObj];
          return prev;
        });
    });
    ...
    newSocket.on("space left", () => {
      fetch(`${socketUrl}/api/spaces`, {
        ...
        .catch((err) =>
          console.error("Failed to sync spaces after leave sync", err),
        );
    });
    ...
    newSocket.on(
      "read_receipt_update",
      ({ username: rUser, message_id: rMsgId }) => {
  ...
  const fetchPinnedMessages = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${socketUrl}/api/spaces/${currentSpace.id}/pinned`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
      ...
      processedData = await Promise.all(
        data.map(async (msg) => {
          ...
          const d = await tryDecryptMsg(
            msg.text,
            activeKeysRef.current[currentSpace.id],
          );
          return { ...msg, ...d };
        }),
      );
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting) {
          ...
          fetch(
            `${socketUrl}/api/spaces/${currentSpace.id}/messages?before_id=${oldestId}`,
            {
              headers: { Authorization: `Bearer ${token}` },
            },
          )
  ...
  const inviteToRoom = async (e) => {
    ...
    if (privateKeyRef.current && activeKeysRef.current[currentSpace.id]) {
      try {
        ...
        alert(
          "E2EE Setup Failed for invitations. Ensure all users have valid profiles.",
        );
    ...
    try {
      const res = await fetch(
        `${socketUrl}/api/spaces/${currentSpace.id}/invite`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          ...
          body: JSON.stringify({
            invited_users: roomSettingsInvitedUsers,
            keyShares,
          }),
        },
      );
    ...
    try {
      ...
      if (authMode === "register") {
        try {
          ...
          payload.wrappedPrivateKey = await wrapPrivateKey(
            keyPair.privateKey,
            authPassword,
            authEmail,
          );
      ...
      if (
        ...
        if (data.wrapped_private_key) {
          try {
            ...
            const exportedPrivateJwk = await window.crypto.subtle.exportKey(
              "jwk",
              privateKey,
            );
            ...
            console.warn(
              "[E2EE] Private key unwrap failed, regenerating identity keys...",
              unwrapErr,
            );
            try {
              const newKeyPair = await generateIdentityKeyPair();
              const newPubKey = await exportPublicKey(newKeyPair);
              const newWrapped = await wrapPrivateKey(
              ...
              await fetch(`${socketUrl}/api/profile`, {
                ...
                body: JSON.stringify({
                  ...
                  wrapped_private_key: newWrapped,
                }),
              });
        ...
        if (backendPalette === "#d0bcff" || !backendPalette.startsWith("#"))
          backendPalette = "#4CAF50";
  ...
  const fetchProfile = async (currentToken) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setColorPalette(
          (data.color_palette === "#d0bcff" ? "#4CAF50" : data.color_palette) ||
            "#4CAF50",
        );
  ...
  useEffect(() => {
    if (window._pendingDeepSpaceId && spaces.length > 0) {
      const target = spaces.find(
        (s) => Number(s.id) === Number(window._pendingDeepSpaceId),
      );
      if (target) {
        setCurrentSpace(target);
        setMobileView("chat");
    ...
    const checkJoinLink = async () => {
      ...
      if (path.startsWith("/join/") && token && isConnected) {
        ...
        if (inviteKey) {
          try {
            const res = await fetch(
              `${socketUrl}/api/spaces/join/${inviteKey}`,
              {
                method: "POST",
                headers: { Authorization: `Bearer ${token}` },
              },
            ...
            if (res.ok || data.message === "Already a member") {
              if (data.space) {
                setSpaces((prev) => {
                  if (!prev.find((s) => s.id === data.space.id))
                    return [...prev, data.space];
                  return prev;
                });
  ...
  const createSpace = async (e) => {
    ...
    if (privateKeyRef.current && profileData.public_key) {
      try {
        ...
        const ourEncrypted = await encryptRoomKeyWithPublicKey(
          roomKeyObj,
          profileData.public_key,
        );
        ...
        alert(
          "E2EE Setup Failed: Check console. Make sure user profiles are loaded.",
        );
  ...
  const removeUserFromSpace = async (userId) => {
    ...
    try {
      const res = await fetch(
        `${socketUrl}/api/spaces/${currentSpace.id}/remove`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const searchGiphy = async () => {
    ...
    try {
      const res = await fetch(
        `${socketUrl}/api/gifs?q=${encodeURIComponent(gifSearch)}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const sendMessage = async (e) => {
    ...
    if ((input.trim() || pendingAsset) && socket && isConnected) {
      ...
      if (isEncryptedSpace(spaceObj) && !activeKeys[spaceObj.id]) {
        alert(
          "Encryption Key missing. Security context unavailable. Message blocked.",
        );
        return;
      }
      ...
      const allKnownUsers = (allUsers || []).concat(
        onlineUsers.filter(
          (u) => !(allUsers || []).some((a) => a.username === u.username),
        ),
      );
      ...
      if (payloadText.trim()) {
        try {
          if (isEncryptedSpace(spaceObj)) {
            payloadText = await encryptMessage(
              payloadText,
              activeKeys[spaceObj.id],
            );
          }
        } catch (err) {
  ...
  const sendInlineReply = async (parentMsgId) => {
    if (!inlineReplyText.trim() || !socket || !isConnected) return;
    let payloadText = inlineReplyText.trim();
    const spaceObj = currentSpace;
    if (isEncryptedSpace(spaceObj)) {
      if (!activeKeys[spaceObj.id]) {
        alert("Encryption key missing. Cannot send reply.");
        return;
      }
      try {
        payloadText = await encryptMessage(
  ...
  const insertMention = (user) => {
    ...
    textNode.textContent =
      before.substring(0, atIdx) + `@${displayName} ` + after;
    const newRange = document.createRange();
  ...
  const getFilteredMentionUsers = () => {
    return onlineUsers
      .concat(
        (allUsers || []).filter(
          (u) => !onlineUsers.some((o) => o.username === u.username),
        ),
      )
      .filter((u) => {
        ...
        return (
          displayName.includes(mentionFilter) ||
  ...
  const getSpaceDisplayName = (spaceObj = currentSpace) => {
    ...
    if (spaceObj.name.startsWith("self_"))
      return `${profileData.first_name || username} (Notes to Self)`;
        ...
        >
          ...
          {threadToast && (
            <div
              onClick={() => {
                const scrollToMsg = (msgId) => {
                  ...
                  if (!el) return false;
                  const container = el.closest(".chat-window");
                  if (container) {
                ...
                if (threadToast.crossSpaceId) {
                  const targetSpace = spaces.find(
                    (s) => s.id === threadToast.crossSpaceId,
                  );
                  if (targetSpace) {
                    handleSpaceSelect(targetSpace);
                    const parentId = threadToast.parentMsgId;
                    const tryScroll = (attempts = 0) => {
                      if (!scrollToMsg(parentId) && attempts < 15) {
                  ...
                  scrollToMsg(threadToast.parentMsgId);
                }
                setThreadToast(null);
                setShowSidebar(false);
                setMobileView("chat");
              ...
              style={{
                ...
                cursor: "pointer",
                animation:
                  "slideDownFade 0.35s cubic-bezier(0.175, 0.885, 0.32, 1.275)",
                fontSize: "0.85rem",
                fontWeight: 500,
                maxWidth: "90vw",
              }}
            >
              ...
              <span>
                <strong>{threadToast.senderName}</strong> replied to your
                message
                {threadToast.crossSpaceName
                  ? ` in #${threadToast.crossSpaceName}`
                  : threadToast.crossIsDm
                    ? " in a DM"
                    : ""}
              </span>
              <button
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        const allMessages = [
                          ...messages.map((m) => ({
                            ...m,
                            spaceId: m.spaceId || currentSpaceId,
                          })),
                        ];
                        // Add cached messages from other spaces
                        Object.entries(messageCacheRef.current).forEach(
                          ([sid, msgs]) => {
                      ...
                      onKeyDown={(e) => {
                        ...
                        if (e.key === "Enter" && searchResults.length > 0) {
                          ...
                          if (result) {
                            const targetSpace = spaces.find(
                              (s) => s.id === result.spaceId,
                            );
                            if (
                              targetSpace &&
                              targetSpace.id !== currentSpace?.id
                            )
                              handleSpaceSelect(targetSpace);
                            setTimeout(
                              () =>
                      ...
                      <div className="global-search-results">
                        {searchResults.length === 0 ? (
                          ...
                          searchResults.slice(0, 20).map((r, i) => {
                            ...
                            const spaceName = matchedSpace
                              ? getSpaceDisplayName(matchedSpace)
                              : "Unknown";
                            const senderDisplay = r.first_name
              ...
              >
                ...
                {profileData.status_emoji &&
                  profileData.status_emoji !== "none" && (
                    ...
                    >
                      <StatusIcon
                        statusId={profileData.status_emoji}
                        size={12}
                      />
                    </div>
                  )}
          ...
          <div className={`sidebar ${showSidebar ? "open" : ""}`}>
            ...
            {/* Start Chat Button */}
            <div
              style={{ padding: "16px 20px 8px 20px", position: "relative" }}
              ref={startChatRef}
            >
              <button
                className="btn-primary"
                    ...
                    onClick={() => {
                      ...
                      if (allUsers.length === 0)
                        fetch(`${socketUrl}/api/users`, {
                          ...
                          .then((data) =>
                            setAllUsers(
                              data.filter((u) => u.username !== username),
                            ),
                          );
                      setShowSpaceModal(true);
                    }}
            ...
            <div className="space-list" style={{ flex: 1, overflowY: "auto" }}>
                    ...
                    >
                      ...
                      {mentionSpaces[space.id] > 0 && (
                        <span
                          ...
                          title={`${mentionSpaces[space.id]} mention(s)`}
                        >
                          @
                        </span>
                      )}
                    ...
                    >
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          setActiveSpaceMenu(
                            activeSpaceMenu === space.id ? null : space.id,
                          );
                        }}
                        title="Space Options"
                      >
              ...
              {spaces
                ...
                .map((space) => {
                  ...
                  const isOnline =
                    isSelf ||
                    onlineUsers.some((u) => u.username === space.dm_username);
                      ...
                      >
                        <ProfileHoverCard
                          userData={allUsers.find(
                            (u) => u.username === space.dm_username,
                          )}
                          isSelf={isSelf}
                                ...
                                <div
                                  style={{
                                    ...
                                    backgroundColor: isOnline
                                      ...
                                      : "var(--md-sys-color-outline-variant)",
                                    border:
                                      "2px solid var(--md-sys-color-surface)",
                                    zIndex: 10,
                                  }}
                                ></div>
                              );
                        ...
                        >
                          ...
                          {(() => {
                            ...
                            return se && se !== "none" ? (
                              <StatusIcon statusId={se} size={13} />
                            ) : null;
                          })()}
                          {unreadCounts[space.id] > 0 && (
                            <span
              ...
              <>
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    >
                      {getSpaceDisplayName()}
                      {currentSpace.is_private === 1 &&
                        currentSpace.is_dm !== 1 && (
                          <svg
                            width="16"
                            height="16"
                            viewBox="0 0 24 24"
                      ...
                      currentSpace.is_dm !== 1 && (
                        <button
                          onClick={() => {
                            ...
                            fetch(
                              `${socketUrl}/api/spaces/${currentSpace.id}/members`,
                              {
                                headers: { Authorization: `Bearer ${token}` },
                              },
                            )
                ...
                <main className="chat-window">
                  ...
                  {incomingCall &&
                    ...
                    (() => {
                      ...
                      const acceptCall = (audioOnly) => {
                        const targetSpace = spaces.find(
                          (s) => Number(s.id) === Number(incomingCall.spaceId),
                        );
                        if (targetSpace) {
                          handleSpaceSelect(targetSpace);
                        }
                      ...
                      const spaceName = (() => {
                        const s = spaces.find(
                          (sp) =>
                            Number(sp.id) === Number(incomingCall.spaceId),
                        );
                        if (!s) return "";
                        if (s.is_dm === 1) return "";
                        return s.name;
                  ...
                  {/* Pushes messages to bottom if few */}
                  <div
                    ref={topAnchorRef}
                    style={{ width: "100%", height: "1px", flexShrink: 0 }}
                  ></div>
                  {isFetchingHistory && (
                    <div
                      style={{
                          ...
                          <React.Fragment key={msg.id || idx}>
                            ...
                            >
                              ...
                              <div
                                ...
                                onClick={() =>
                                  setShowTimestampId(
                                    showTimestampId === msg.id ? null : msg.id,
                                  )
                                }
                                style={{ cursor: "pointer" }}
                              >
                                {Object.entries(readReceipts).filter(
                                  ([u, id]) => id === msg.id && u !== username,
                                ).length > 0 && (
                                  <div
                                    style={{
                                      position: "absolute",
                                  ...
                                  >
                                    {Object.entries(readReceipts)
                                      .filter(
                                        ([u, id]) =>
                                          id === msg.id && u !== username,
                                      )
                                      .map(([u]) => {
                                        let av = null;
                                        let displayName = u;
                                        const ou = onlineUsers.find(
                                          (o) => o.username === u,
                                        );
                                        if (ou) {
                                        ...
                                        if (!av || displayName === u) {
                                          for (
                                            ...
                                            if (messages[i].sender === u) {
                                              if (!av && messages[i].avatar)
                                                av = messages[i].avatar;
                                              if (
                                                displayName === u &&
                                                messages[i].first_name
                                              )
                                                displayName =
                                ...
                                >
                                  ...
                                  {(role === "admin" ||
                                    (currentSpace &&
                                      currentSpace.created_by === username)) &&
                                    !msg.asset && (
                                      <button
                                        onClick={() =>
                                          pinMessage(
                                    ...
                                    <button
                                      onClick={() => {
                                        ...
                                        if (reason && reason.trim()) {
                                          fetch(
                                            `${socketUrl}/api/messages/${msg.id}/report`,
                                            {
                                              method: "POST",
                                              headers: {
                                                "Content-Type":
                                ...
                                {msg.asset && (
                                  <div className="message-asset">
                                    ...
                                    msg.asset.match(
                                      /\.(jpeg|jpg|gif|png|webp|heic|heif|bmp|svg|tiff|tif|ico)$/i,
                                    ) ? (
                                      <img
                                        src={
                                      ...
                                      <div
                                        ...
                                        ref={(el) => {
                                          if (!el || el.dataset.audioInit)
                                            return;
                                          el.dataset.audioInit = "true";
                                          const audio =
                                            document.createElement("audio");
                                          const src = msg.asset.startsWith(
                                            "/uploads/",
                                          )
                                            ? `${socketUrl}${msg.asset}`
                                            : msg.asset;
                                      ...
                                      msg.asset.match(
                                        /\.(mp4|webm|ogg|mov|qt|3gp|avi|wmv|flv|m4v|mpg|mpeg)$/i,
                                      ) ||
                                              ...
                                              >
                                                ...
                                                <span>
                                                  {(() => {
                                                    try {
                                                      return new URL(
                                                        unfurlData[msg.id].url,
                                                      ).hostname;
                                                    } catch {
                                                      return unfurlData[msg.id]
                                                        .url;
                                                    }
                                                  })()}
                                                </span>
                                            ...
                                            {Object.entries(
                                              ...
                                              <div
                                                ...
                                                style={{
                                                  ...
                                                  gap: "4px",
                                                  backgroundColor:
                                                    usersArr.includes(username)
                                                      ? "var(--md-sys-color-primary-container)"
                                                      : "var(--md-sys-color-surface-variant)",
                                                  color: usersArr.includes(
                                                    username,
                                                  )
                                                    ? "var(--md-sys-color-on-primary-container)"
                                                    : "var(--md-sys-color-on-surface-variant)",
                                    ...
                                    >
                                      ...
                                      <button
                                        onClick={() => {
                                          ...
                                          // Clear thread unread badge when expanding
                                          if (isCollapsed) {
                                            setThreadUnreads((prev) => {
                                              const next = { ...prev };
                                              delete next[msg.id];
                                      ...
                                      >
                                        ...
                                        >
                                          <textarea
                                            ...
                                            placeholder={`Reply to ${msg.first_name || msg.sender}...`}
                                            style={{
                                              width: "100%",
                                              minHeight: "36px",
                                              maxHeight: "120px",
                                              ...
                                              style={{
                                                background:
                                                  inlineReplyText.trim()
                                                    ? "var(--md-sys-color-primary)"
                                                    : "var(--md-sys-color-surface-variant)",
                                                color: inlineReplyText.trim()
                                                  ? "var(--md-sys-color-on-primary)"
                                                  : "var(--md-sys-color-outline)",
                                                border: "none",
                                                borderRadius: "16px",
                                                padding: "4px 14px",
                                                cursor: inlineReplyText.trim()
                                                  ? "pointer"
                                                  : "default",
                                                fontSize: "0.78rem",
                                                fontWeight: 600,
                            ...
                            {/* ── Inline reply for messages without existing thread ── */}
                            {inlineReplyThreadId === msg.id &&
                              (!childrenMap[msg.id] ||
                                childrenMap[msg.id].length === 0) && (
                                <div
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    >
                                      ...
                                      <button
                                        onClick={() => sendInlineReply(msg.id)}
                                        style={{
                                          background: inlineReplyText.trim()
                                            ? "var(--md-sys-color-primary)"
                                            : "var(--md-sys-color-surface-variant)",
                                          color: inlineReplyText.trim()
                                            ? "var(--md-sys-color-on-primary)"
                  ...
                  {typingUsers
                    .filter(
                      (u) => Number(u.spaceId) === Number(currentSpace.id),
                    )
                    .map((typer) => (
                      <div
                        key={typer.username}
                        ...
                        style={{
                          animation:
                            "slideDownFade 0.2s cubic-bezier(0.175, 0.885, 0.32, 1.275)",
                        }}
                      >
                        {typer.avatar ? (
                          <img
                  ...
                  {newMsgPill && (
                    <div
                      ...
                      style={{
                        ...
                        cursor: "pointer",
                        animation:
                          "slideUpFade 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275)",
                        fontSize: "0.82rem",
                        fontWeight: 500,
                        maxWidth: "85vw",
                        whiteSpace: "nowrap",
                    ...
                    >
                      ...
                      >
                        <strong>{newMsgPill.senderName}</strong>
                        {newMsgPill.count > 1 && (
                          <span
                            style={{
                              display: "inline-flex",
                        ...
                        {newMsgPill.count === 1 && newMsgPill.lastText && (
                          <span style={{ opacity: 0.7, marginLeft: "6px" }}>
                            {newMsgPill.lastText.length > 40
                              ? newMsgPill.lastText.slice(0, 40) + "…"
                              : newMsgPill.lastText}
                          </span>
                        )}
                      </span>
                  ...
                  {!isUploadingMedia && pendingAsset && (
                    <div className="asset-preview">
                      {pendingAsset.match(
                        ...
                        <img
                          src={
                            pendingAsset.startsWith("/uploads")
                              ? `${socketUrl}${pendingAsset}`
                              : pendingAsset
                          }
                          alt="Preview"
                        />
                        ...
                        <video
                          src={
                            pendingAsset.startsWith("/uploads")
                              ? `${socketUrl}${pendingAsset}#t=0.001`
                              : pendingAsset
                          }
                          style={{
                            maxWidth: "100%",
                    ...
                    {!isRecording && (
                      <>
                        ...
                        <div
                          ...
                          onKeyUp={handleTextSelect}
                          onBlur={() =>
                            setTimeout(() => setFormatToolbar(null), 200)
                          }
                          onPaste={(e) => {
                            e.preventDefault();
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <strong>#{spaceToDelete.name}</strong>? All messages inside
                  this space will be wiped forever.
                </p>
                <div
                  style={{
                    display: "flex",
                    ...
                    <div className="settings-section">
                        ...
                        >
                          ...
                          <label
                            style={{
                              ...
                              cursor: "pointer",
                              border:
                                "2px solid var(--md-sys-color-surface-variant)",
                              transition: "transform 0.15s",
                      ...
                      >
                        ...
                        <input
                          ...
                          onFocus={() => {
                            if (profileData.location?.length >= 2)
                              setShowSuggestions(true);
                          }}
                          placeholder="City, State"
                        />
                          ...
                          profileData.status_emoji !== "none" && (
                            ...
                            >
                              ...
                              <input
                                ...
                                placeholder={
                                  STATUS_ICONS.find(
                                    (s) => s.id === profileData.status_emoji,
                      ...
                      <div style={{ marginTop: "12px" }}>
                        ...
                        <div className="settings-row" style={{ gap: "8px" }}>
                          ...
                          >
                            ...
                            >
                              {(profileData.timezone || "Not set").replace(
                                /_/g,
                                " ",
                              )}
                            </span>
                  ...
                  <div className="settings-col">
                    ...
                    <div className="settings-section">
                      ...
                      >
                        ...
                        <FontPicker
                          value={
                            profileData.font_family || globalFont || "Inter"
                          }
                          onApply={(val) => {
                            setProfileData({
                              ...profileData,
                    ...
                    <div className="settings-section">
                      ...
                      <button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            const res = await fetch(
                              `${socketUrl}/api/profile/password`,
                              {
                                method: "PUT",
                                headers: {
                                  "Content-Type": "application/json",
                                ...
                                body: JSON.stringify({
                                  currentPassword: pwChange.current,
                                  newPassword: pwChange.next,
                                }),
                              },
                            );
                            const data = await res.json();
                            setPwChange(
                              res.ok
                                ...
                                : (p) => ({
                                    ...p,
                                    msg: { ok: false, text: data.error },
                                  }),
                            );
                            if (res.ok)
                              setTimeout(
                                () => setPwChange((p) => ({ ...p, msg: null })),
                                3000,
                              );
                          } catch {
                            setPwChange((p) => ({
                              ...p,
                              msg: { ok: false, text: "Network error" },
                            }));
                          }
                        }}
                      >
          ...
          {showCallInvite && currentSpace && (
            <div
              ...
              onClick={(e) => {
                if (e.target.className === "space-modal-overlay")
                  ...
                  >
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={
                          callInviteSelected.length === callInviteMembers.length
                        }
                        onChange={(e) =>
                          setCallInviteSelected(
                            e.target.checked
                              ? callInviteMembers.map((m) => m.id)
                              : [],
                          )
                        }
                        style={{
          ...
          {selectedAsset && (
            ...
            >
              {selectedAsset.match(
                /\.(mp4|mov|webm|mkv|3gp|avi|wmv|flv|m4v|mpg|mpeg)$/i,
                ...
                />
                  ...
                  /\.(jpg|jpeg|png|gif|webp|heic|heif|bmp|svg|tiff|tif|ico)$/i,
                ) ? (
                <img
                  src={selectedAsset}
                  alt="Full Size"
                ...
                >
                  ...
                  >
                    This document type cannot be natively previewed in the
          ...
          {showRoomSettingsModal && (
            ...
            >
              ...
              >
                ...
                >
                  Select members to directly invite to{" "}
                  <strong>#{currentSpace.name}</strong>.
                </p>

                <form
                  onSubmit={inviteToRoom}
                ...
                >
                            ...
                            >
                              ...
                              >
                                <input
                                  ...
                                  checked={
                                    isInvited ||
                                    roomSettingsInvitedUsers.includes(u.id)
                                  }
                                  disabled={isInvited}
                                  onChange={(e) => {
                                    if (isInvited) return;
                                    if (e.target.checked)
                                      setRoomSettingsInvitedUsers((prev) => [
                                        ...prev,
                                        u.id,
                                      ]);
                                    else
                              ...
                              {isInvited &&
                                (role === "admin" ||
                                  username === currentSpace.created_by) && (
                                  <button
                                    type="button"
                  ...
                  >
                    ...
                    <button
                      ...
                      className="btn-primary"
                      disabled={
                        isUpdatingRoom || roomSettingsInvitedUsers.length === 0
                      }
                      style={{ flex: 1, padding: "0.75rem" }}
                    >
          ...
          {showSpaceModal && (
            ...
            >
              ...
              >
                ...
                >
                  Spaces are where your team communicates. They're best when
                  ...
                  onSubmit={async (e) => {
                    ...
                    try {
                      ...
                      if (
                        ...
                        keyShares[exactUserId] =
                          await encryptRoomKeyWithPublicKey(
                            roomKey,
                            profileData.public_key,
                          );
                      }
                      ...
                      await Promise.all(
                        invitedUsers.map(async (uId) => {
                          const uObj = allUsers.find(
                            (u) => u.id === Number(uId),
                          );
                          if (uObj && uObj.public_key) {
                            keyShares[uId] = await encryptRoomKeyWithPublicKey(
                              roomKey,
                              uObj.public_key,
                            );
                          }
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        const res = await fetch(`${socketUrl}/api/dms`, {
                          ...
                          body: JSON.stringify({
                            targetUserId:
                              spaces.find((s) => s.name.startsWith("self_"))
                                ?.dm_username !== undefined
                                ? 0 /* mock */
                                : 0,
                          }),
                        ...
                        const currentSelfDm = spaces.find(
                          (s) => s.is_dm === 1 && s.name.startsWith("self_"),
                        );
                        if (currentSelfDm) {
          ...
          {spaceToDelete && (
            ...
            >
              <div
                className="auth-card modal-compact"
                style={{ maxWidth: "400px", textAlign: "center" }}
              >
                <h2 style={{ color: "var(--md-sys-color-error)" }}>
                  Delete Space?
