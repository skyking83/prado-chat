    ...
    db.serialize(() => {
      db.run(
        ...
        () => {
          ...
          db.run(
            `ALTER TABLE spaces ADD COLUMN is_dm INTEGER DEFAULT 0`,
            () => {},
          );
      ...
      db.run(
        ...
        (err) => {
          ...
          if (!err) {
            ...
            db.run(
              `ALTER TABLE users ADD COLUMN color_palette TEXT DEFAULT '#4CAF50'`,
              () => {},
            );
      ...
      db.run(
        ...
        () => {
          db.run(
            `INSERT OR IGNORE INTO app_settings (key, value) VALUES ('global_font', 'Roboto')`,
          );
        },
      );
      ...
      db.run(
        ...
        () => {
          db.run(
            `ALTER TABLE messages ADD COLUMN space_id INTEGER DEFAULT 1`,
            () => {},
          );
          ...
          db.run(`DELETE FROM messages WHERE space_id = 1`, (err) => {
            if (err)
              console.error("Error deleting General messages:", err.message);
          });
          db.run(`DELETE FROM space_members WHERE space_id = 1`, (err) => {
            if (err)
              console.error("Error deleting General members:", err.message);
          });
          db.run(`DELETE FROM spaces WHERE id = 1`, (err) => {
            if (err)
              console.error("Error deleting General space:", err.message);
          });
          ...
          db.all(`SELECT id, username FROM users`, [], (err, usersList) => {
            if (!err && usersList) {
              usersList.forEach((u) => {
                db.get(
                  `SELECT s.id FROM spaces s JOIN space_members sm ON s.id = sm.space_id WHERE s.is_dm = 1 AND s.name LIKE 'self_%' AND sm.user_id = ?`,
                  [u.id],
                  (err, row) => {
                    if (!row) {
                      db.run(
                        `INSERT INTO spaces (name, created_by, is_private, is_dm) VALUES (?, ?, 1, 1)`,
                        [`self_${u.id}_${Date.now()}`, u.username],
                        function (err) {
                          if (!err && this.lastID) {
                            db.run(
      ...
      db.run(
        ...
        () => {
          ...
          const defaults = [
            ...
            [
              "maintenance_message",
              "System is undergoing maintenance. Please check back soon.",
            ],
          ];
...
app.get("/api/config/public", (req, res) => {
  db.all(
    'SELECT key, value FROM config WHERE key IN ("app_name", "registration_mode", "maintenance_mode", "maintenance_message", "default_theme", "default_accent_color", "custom_logo")',
    [],
    (err, rows) => {
      if (err) return res.status(500).json({ error: "Database error" });
      const config = {};
...
app.post("/api/register", async (req, res) => {
  ...
  if (!password || !email || !publicKey || !wrappedPrivateKey) {
    return res
      .status(400)
      .json({ error: "Email, password, and E2EE keys required" });
  }
  try {
      ...
      if (allowed.length > 0) {
        ...
        if (!allowed.includes(domain)) {
          return res
            .status(403)
            .json({
              error: `Registration is restricted to: ${allowed.join(", ")}`,
            });
        }
      }
    }
    ...
    db.get("SELECT COUNT(*) as count FROM users", [], async (err, row) => {
      ...
      const requireVerify = await getConfig(
      ...
      db.run(
        "INSERT INTO users (username, email, password_hash, role, is_verified, verification_token, public_key, wrapped_private_key) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        [
          username,
          email,
          hash,
        ...
        async function (err) {
          ...
          db.run(
            `INSERT INTO spaces (name, created_by, is_private, is_dm) VALUES (?, ?, 1, 1)`,
            [`self_${newUserId}_${Date.now()}`, username],
            function (err) {
              if (!err && this.lastID) {
                db.run(
                  `INSERT INTO space_members (space_id, user_id) VALUES (?, ?)`,
                  [this.lastID, newUserId],
                );
              }
            },
          );
          ...
          if (isVerified === 0) {
            ...
            try {
              const fromAddr = await getConfig(
                "email_from",
              ...
              if (!resendClient) {
                ...
                return res
                  ...
                  .json({
                    message:
                      "Account created but email verification could not be sent. Contact admin.",
                    role,
                  });
              }
              ...
              res
                .status(500)
                .json({
                  error: "Failed to send verification email. Contact admin.",
                });
            }
          } else {
            res
              .status(201)
              .json({ message: "Admin account generated successfully.", role });
          }
        },
      );
    });
...
app.post("/api/login", (req, res) => {
  ...
  db.get("SELECT * FROM users WHERE email = ?", [email], async (err, user) => {
    ...
    if (user.suspended) {
      return res
        .status(403)
        .json({ error: "Account suspended. Contact administrator." });
    }
  ...
  db.get(
    ...
    async (err, user) => {
      ...
      try {
        ...
        db.run(
          "UPDATE users SET password_hash = ?, reset_token = NULL, reset_token_expires = NULL, is_verified = 1 WHERE id = ?",
          [hash, user.id],
          (updateErr) => {
            if (updateErr)
              return res.status(500).json({ error: "Database error" });
            res
              .status(200)
              .json({ message: "Password reset successfully completed" });
          },
        );
      } catch (hashErr) {
        res.status(500).json({ error: "Server error" });
...
const authenticateToken = (req, res, next) => {
  ...
  jwt.verify(token, JWT_SECRET, (err, user) => {
    ...
    db.get(
      "SELECT id, suspended FROM users WHERE id = ?",
      [user.userId],
      (dbErr, dbUser) => {
        if (dbErr || !dbUser)
...
app.get("/api/profile", authenticateToken, (req, res) => {
  db.get(
    "SELECT id, username, role, theme, color_palette, avatar, first_name, last_name, email, location, font_family, bio, status_text, status_emoji, timezone, public_key FROM users WHERE id = ?",
    [req.user.userId],
    (err, user) => {
      if (err) return res.status(500).json({ error: "Database error" });
...
app.get("/api/users", authenticateToken, (req, res) => {
  db.all(
    "SELECT id, username, avatar, first_name, last_name, public_key, status_text, status_emoji, timezone FROM users ORDER BY username ASC",
    [],
    (err, rows) => {
      if (err) return res.status(500).json({ error: "Database error" });
      res.json(rows);
...
app.put("/api/profile", authenticateToken, (req, res) => {
  ...
  db.run(
    ...
    function (err) {
      ...
      db.get(
        "SELECT username, avatar, font_family, location, status_text, status_emoji FROM users WHERE id = ?",
        [req.user.userId],
        (dbErr, row) => {
          if (!dbErr && row) {
...
function logAudit(
  ...
  db.run(
    "INSERT INTO audit_log (admin_id, admin_username, action, target_type, target_id, details) VALUES (?, ?, ?, ?, ?, ?)",
    [
      adminId,
      adminUsername,
      action,
...
app.get("/api/admin/users", authenticateToken, requireAdmin, (req, res) => {
  db.all(
    "SELECT id, username, role, first_name, last_name, email, location, avatar, theme, color_palette, font_family, bio, status_text, status_emoji, timezone, public_key, created_at, suspended FROM users ORDER BY id ASC",
    [],
    (err, rows) => {
      if (err) return res.status(500).json({ error: "Database error" });
      res.json(rows);
...
app.get(
  ...
  (req, res) => {
    ...
    db.all(
      "SELECT * FROM login_history WHERE user_id = ? ORDER BY timestamp DESC LIMIT ? OFFSET ?",
...
app.get(
  ...
  (req, res) => {
    ...
    db.all(
      "SELECT id, username, email, role, first_name, last_name, location, bio, suspended, created_at FROM users ORDER BY id",
      [],
      (err, rows) => {
        if (err) return res.status(500).json({ error: "Database error" });
        if (format === "csv") {
...
app.post(
  ...
  (req, res) => {
    ...
    db.run(
      `UPDATE users SET suspended = ? WHERE id IN (${placeholders})`,
      [suspend ? 1 : 0, ...filtered],
      function (err) {
        if (err) return res.status(500).json({ error: "Update failed" });
...
app.post("/api/messages/:id/report", authenticateToken, (req, res) => {
  ...
  db.get(
    "SELECT text, sender FROM messages WHERE id = ?",
    [messageId],
    (err, msg) => {
      ...
      db.get(
        "SELECT id FROM message_reports WHERE reporter_id = ? AND message_id = ? AND status = ?",
        [req.user.userId, messageId, "pending"],
        (err, existing) => {
          if (existing)
            return res.status(409).json({ error: "Already reported" });

          db.run(
            "INSERT INTO message_reports (reporter_id, reporter_username, message_id, space_id, message_text, message_sender, reason) VALUES (?, ?, ?, ?, ?, ?, ?)",
            [
              req.user.userId,
              req.user.username,
              messageId,
...
app.get("/api/admin/reports", authenticateToken, requireAdmin, (req, res) => {
  ...
  db.all(
    "SELECT * FROM message_reports WHERE status = ? ORDER BY created_at DESC LIMIT ? OFFSET ?",
    [status, limit, offset],
    (err, rows) => {
      if (err) return res.status(500).json({ error: "Database error" });
      db.get(
        "SELECT COUNT(*) as total FROM message_reports WHERE status = ?",
        [status],
        (err2, c) => {
          db.get(
            "SELECT COUNT(*) as pending FROM message_reports WHERE status = ?",
            ["pending"],
            (err3, p) => {
              res.json({
                reports: rows || [],
                total: c?.total || 0,
                pendingCount: p?.pending || 0,
              });
            },
          );
        },
...
app.put(
  ...
  (req, res) => {
    ...
    db.run(
      'UPDATE message_reports SET status = ?, resolved_by = ?, resolved_at = datetime("now") WHERE id = ?',
      ...
      function (err) {
        ...
        if (status === "resolved" && req.body.deleteMessage) {
          db.get(
            "SELECT message_id, space_id FROM message_reports WHERE id = ?",
            [req.params.id],
            (err, report) => {
              if (report) {
...
app.get(
  ...
  (req, res) => {
    db.all(
      "SELECT * FROM word_filters ORDER BY created_at DESC",
      [],
      (err, rows) => {
        if (err) return res.status(500).json({ error: "Database error" });
        res.json(rows || []);
...
app.post(
  ...
  (req, res) => {
    ...
    db.run(
      "INSERT OR IGNORE INTO word_filters (pattern, action) VALUES (?, ?)",
      [pattern.toLowerCase().trim(), filterAction],
      function (err) {
        if (err) return res.status(500).json({ error: "Insert failed" });
        if (this.changes === 0)
          return res.status(409).json({ error: "Filter already exists" });
        logAudit(
          req.user.userId,
          req.user.username,
          "add_word_filter",
...
app.get("/api/admin/api-keys", authenticateToken, requireAdmin, (req, res) => {
  db.all(
    "SELECT id, name, key_prefix, permissions, created_by, last_used, created_at FROM api_keys ORDER BY created_at DESC",
    [],
    (err, rows) => {
      if (err) return res.status(500).json({ error: "Database error" });
      res.json(rows || []);
...
app.post("/api/admin/api-keys", authenticateToken, requireAdmin, (req, res) => {
  ...
  const perm = ["read", "write", "admin"].includes(permissions)
    ? permissions
    : "read";

  db.run(
    "INSERT INTO api_keys (name, key_hash, key_prefix, permissions, created_by) VALUES (?, ?, ?, ?, ?)",
    [name, keyHash, keyPrefix, perm, req.user.username],
    function (err) {
    ...
    try {
      ...
      if (!apiKey)
        return res
          ...
          .json({
            error:
              "No Resend API key configured. Set it in Config → Email Provider.",
          });
      ...
      const result = await testResend.emails.send({
        ...
        html:
          '<h2>✅ Email Configuration Working</h2><p>If you received this email, your Prado Chat email provider is configured correctly.</p><p style="color: #888; font-size: 12px;">Sent at ' +
      ...
      if (result?.error)
        return res
          .status(500)
          .json({ error: result.error.message || "Resend API error" });
      logAudit(
        req.user.userId,
        req.user.username,
        "test_email",
...
app.get(
  ...
  async (req, res) => {
    ...
    const resendKey = await getConfig(
      "resend_api_key",
      process.env.RESEND_API_KEY || "",
    );
    const giphyKey = await getConfig(
      "giphy_api_key",
...
app.get("/api/ice-servers", authenticateToken, async (req, res) => {
  ...
  const stunServer = await getConfig(
    "stun_server",
...
app.get("/api/admin/assets", authenticateToken, requireAdmin, (req, res) => {
  fs.readdir(UPLOADS_DIR, (err, files) => {
    ...
    const fileStats = files.map((file) => {
      ...
      const isVoiceFile = file.includes("voice-");
      let type = "application/octet-stream";
      if (isVoiceFile || ext.match(/\.(mp3|ogg|wav|m4a|aac|flac|wma)$/))
        type = `audio/${ext.replace(".", "") || "webm"}`;
      else if (
        ext.match(/\.(jpg|jpeg|png|gif|webp|heic|heif|bmp|svg|tiff|ico)$/)
      )
        type = `image/${ext.replace(".", "").replace("jpg", "jpeg")}`;
      else if (ext.match(/\.(mp4|mov|webm|mkv|3gp)$/))
        type = `video/${ext.replace(".", "")}`;
      else if (ext === ".pdf") type = "application/pdf";
      else if (ext.match(/\.(ai|eps)$/))
        type = `application/${ext === ".ai" ? "illustrator" : "postscript"}`;
...
app.delete(
  ...
  (req, res) => {
    ...
    if (fs.existsSync(filepath)) {
      ...
      db.all(
        "SELECT id, space_id FROM messages WHERE asset = ?",
        [assetUrl],
        (err, rows) => {
          if (!err && rows && rows.length > 0) {
            db.run("DELETE FROM messages WHERE asset = ?", [assetUrl], () => {
              ...
              rows.forEach((row) => {
                io.to(row.space_id.toString()).emit("message deleted", {
                  id: row.id,
                });
              });
            });
...
app.delete(
  ...
  (req, res) => {
    ...
    if (spaceId == 1)
      return res.status(403).json({ error: "Cannot delete the General space" });

    // Protect Notes to Self spaces from deletion
    db.get(
      "SELECT name, is_dm FROM spaces WHERE id = ?",
      [spaceId],
      (err, space) => {
        if (err || !space)
        ...
        if (space.is_dm === 1 && space.name.startsWith("self_")) {
          return res
            .status(403)
            .json({ error: "Cannot delete Notes to Self spaces" });
        }
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const active24hRow = await dbGet(
        "SELECT COUNT(DISTINCT sender) as count FROM messages WHERE timestamp > ?",
        [d24h],
      );
      const active7dRow = await dbGet(
        "SELECT COUNT(DISTINCT sender) as count FROM messages WHERE timestamp > ?",
        [d7d],
      );

      // Message counts
      const totalMsgsRow = await dbGet(
        "SELECT COUNT(*) as count FROM messages",
      );

      // Total spaces
      const totalSpacesRow = await dbGet(
      ...
      let dbSizeBytes = 0;
      try {
        dbSizeBytes = fs.statSync("./data/database.sqlite").size;
      } catch (_) {}
...
app.post("/api/spaces/:id/escrow-key", authenticateToken, (req, res) => {
  ...
  if (!rawKeyBase64)
    return res.status(400).json({ error: "rawKeyBase64 required" });
  const encrypted = serverEncrypt(rawKeyBase64);
  db.run(
    "UPDATE spaces SET escrow_key = ? WHERE id = ?",
    [encrypted, spaceId],
    (err) => {
      if (err) return res.status(500).json({ error: "Database error" });
      res.json({ success: true });
...
app.get("/api/spaces/:id/request-key", authenticateToken, (req, res) => {
  ...
  db.get(
    "SELECT 1 as ok FROM space_members WHERE space_id = ? AND user_id = ? UNION SELECT 1 as ok FROM spaces WHERE id = ? AND is_private = 0",
    ...
    (err, access) => {
      ...
      db.get(
        "SELECT escrow_key FROM spaces WHERE id = ?",
        [spaceId],
        (err, space) => {
          if (!space?.escrow_key) {
            // Queue the request for when a keyholder connects
            db.run(
              "INSERT OR IGNORE INTO pending_key_requests (space_id, user_id) VALUES (?, ?)",
              [spaceId, userId],
              () => {},
            );
            return res
              .status(404)
              .json({ error: "No escrow key available — request queued" });
          }
          db.get(
            ...
            async (err, user) => {
              if (!user?.public_key)
                return res
                  .status(404)
                  .json({ error: "User has no public key" });
              try {
                ...
                // Store in space_keys
                db.run(
                  "INSERT OR REPLACE INTO space_keys (space_id, user_id, encrypted_room_key) VALUES (?, ?, ?)",
                  [spaceId, userId, wrappedB64],
                  (err) => {
                    if (err)
                      console.error("Failed to store re-wrapped key", err);
                    // Clear any pending request since we fulfilled it
                    db.run(
                      "DELETE FROM pending_key_requests WHERE space_id = ? AND user_id = ?",
                      [spaceId, userId],
                      () => {},
                    );
...
app.get("/api/spaces/keys", authenticateToken, (req, res) => {
  db.all(
    "SELECT space_id, encrypted_room_key FROM space_keys WHERE user_id = ?",
    [req.user.userId],
    (err, rows) => {
      if (err) return res.status(500).json({ error: "Database error" });
      res.json(rows);
...
app.post("/api/spaces", authenticateToken, (req, res) => {
  ...
  db.run(
    "INSERT INTO spaces (name, created_by, is_private, invite_key) VALUES (?, ?, ?, ?)",
    [name.trim(), req.user.username, isPrivate, inviteKey],
    function (err) {
      if (err) {
      ...
      if (keyShares && typeof keyShares === "object") {
        ...
        if (shareUserIds.length > 0) {
          ...
          db.run(
            `INSERT INTO space_keys (space_id, user_id, encrypted_room_key) VALUES ${placeholders}`,
            values,
            (keyErr) => {
      ...
      const broadcastSpace = () => {
        if (!isPrivate) {
          ...
          const allowedIds = [
            req.user.userId,
            ...(Array.isArray(invited_users) ? invited_users : []),
          ];
          // Iterate active sockets checking Auth tokens to push private creation events safely
          io.sockets.sockets.forEach((s) => {
            if (
              s.user &&
              (allowedIds.includes(s.user.userId) || s.user.role === "admin")
            ) {
        ...
        db.run(
          ...
          () => {
            if (Array.isArray(invited_users) && invited_users.length > 0) {
              ...
              const values = [];
              invited_users.forEach((uid) => {
                values.push(spaceId, uid);
              });
...
app.post("/api/spaces/join/:invite_key", authenticateToken, (req, res) => {
  ...
  db.get(
    "SELECT * FROM spaces WHERE invite_key = ?",
    [invite_key],
    (err, space) => {
      if (err) return res.status(500).json({ error: "Database error" });
      if (!space)
        ...
        (err, existing) => {
          ...
          db.run(
            ...
            (err) => {
              ...
              res
                .status(200)
                .json({ message: "Successfully joined private space", space });
            },
          );
        },
      );
...
app.post("/api/spaces/:id/invite", authenticateToken, (req, res) => {
  ...
  console.log(
    `[DEBUG INVITE] Hit invite route for spaceId=${spaceId}. Body:`,
  ...
  db.get("SELECT * FROM spaces WHERE id = ?", [spaceId], (err, space) => {
    ...
    db.run(
      `INSERT OR IGNORE INTO space_members (space_id, user_id) VALUES ${placeholders}`,
      values,
      function (err) {
        ...
        if (keyShares && typeof keyShares === "object") {
          ...
          if (shareUserIds.length > 0) {
            ...
            db.run(
              `INSERT OR IGNORE INTO space_keys (space_id, user_id, encrypted_room_key) VALUES ${kp}`,
              kv,
              function (keyErr) {
                console.log(
                  `[DEBUG INVITE] space_keys insertion completed. changes=${this.changes}, err=`,
                  keyErr,
                );
                if (keyErr)
                  console.error(
                    "Failed to insert space_keys for invites",
                    keyErr,
                  );
              },
            );
...
app.get("/api/messages/:spaceId", authenticateToken, (req, res) => {
  ...
  db.all(
    "SELECT * FROM messages WHERE space_id = ? ORDER BY id ASC LIMIT 100",
    [spaceId],
    (err, rows) => {
      if (err) return res.status(500).json({ error: "Database error" });
      res.json(rows);
...
// ─── URL Unfurl (Open Graph) ───
const unfurlCache = new Map();
app.get("/api/unfurl", authenticateToken, async (req, res) => {
  const { url } = req.query;
  ...
  try {
    ...
    // music.youtube.com → youtube.com (YT Music needs JS, regular YT has SSR OG tags)
    let fetchUrl = url;
    if (/music\.youtube\.com/i.test(parsedUrl.hostname)) {
    ...
    const oEmbedProviders = [
      ...
      {
        pattern:
          /(?:(?:music\.)?youtube\.com\/watch|youtu\.be\/|youtube\.com\/shorts\/)/i,
      ...
      },
      {
        ...
        endpoint: "https://graph.facebook.com/v18.0/oembed_post?url=",
      },
      {
        pattern: /reddit\.com\/r\/.+\/comments\//i,
      ...
      },
      {
        ...
        endpoint: "https://github.com/api/oembed?url=",
      },
      // Media & Design
      {
        pattern: /flickr\.com\/photos\//i,
        endpoint: "https://www.flickr.com/services/oembed?format=json&url=",
      },
      {
        pattern: /giphy\.com\/gifs\//i,
        endpoint: "https://giphy.com/services/oembed?url=",
      },
      {
      ...
      // Other
      {
        ...
        endpoint: "https://www.kickstarter.com/services/oembed?url=",
      },
      {
        pattern: /ted\.com\/talks\//i,
        endpoint: "https://www.ted.com/services/v1/oembed.json?url=",
      },
    ];
    const oEmbedMatch = oEmbedProviders.find((p) => p.pattern.test(fetchUrl));
    if (oEmbedMatch) {
      try {
        const oController = new AbortController();
        const oTimeout = setTimeout(() => oController.abort(), 5000);
        const oRes = await fetch(
          `${oEmbedMatch.endpoint}${encodeURIComponent(fetchUrl)}`,
          {
        ...
        if (oRes.ok) {
          ...
          const data = {
            ...
            siteName: oembed.provider_name || null,
    ...
    // Read up to 200KB (or 2MB for YouTube) and stop early when possible
    const reader = response.body.getReader();
    let html = "";
    let bytesRead = 0;
    const maxBytes = isYouTube ? 2000000 : 200000;
    ...
    while (bytesRead < maxBytes) {
      ...
      if (!isYouTube && html.includes("</head>")) break;
    }
    reader.cancel();

    // Parse OG tags: extract individual <meta> tags first, then search within each
    const metaTags = html.match(/<meta[\s\S]*?\/?>|<meta[\s\S]*?>/gi) || [];
    const getMetaContent = (property) => {
      const escaped = property.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
      const propRegex = new RegExp(`(?:property|name)=["']${escaped}["']`, "i");
      const contentRegex = /content=["']([^"']+)["']/i;
      for (const tag of metaTags) {
        if (propRegex.test(tag)) {
          const m = tag.match(contentRegex);
          if (m) return m[1];
        }
      }
      return null;
    ...
    // YouTube fallback: extract from ytInitialData JSON when OG tags are missing
    if (!data.description && !data.image && /youtube\.com/i.test(fetchUrl)) {
      try {
        const ytMatch = html.match(/var ytInitialData = ({.*?});/s);
        if (ytMatch) {
          const ytData = JSON.parse(ytMatch[1]);
          const ch = ytData?.metadata?.channelMetadataRenderer;
          if (ch) {
            data.title = data.title || ch.title || null;
            data.description = ch.description
              ? ch.description.substring(0, 200)
              : null;
            data.image = ch.avatar?.thumbnails?.slice(-1)?.[0]?.url || null;
    ...
    if (unfurlCache.size > 500) {
      ...
      unfurlCache.delete(oldest);
    }

    res.json(data);
  } catch (e) {
    console.error("Unfurl error:", e.message);
...
app.get("/api/spaces/:id/members", authenticateToken, (req, res) => {
  ...
  db.get(
    "SELECT is_private, is_dm FROM spaces WHERE id = ?",
    [spaceId],
    (err, space) => {
      if (err) return res.status(500).json({ error: "Database error" });
      ...
      if (space.is_private || space.is_dm) {
        ...
        db.all(
          `SELECT u.id, u.username, u.first_name, u.last_name, u.avatar, u.public_key 
        ...
        db.all(
          `SELECT id, username, first_name, last_name, avatar, public_key FROM users`,
          [],
          (err2, rows) => {
            if (err2) return res.status(500).json({ error: "Database error" });
            res.json(rows || []);
...
app.post("/api/dms", authenticateToken, (req, res) => {
  ...
  if (isSelf) {
    db.get(
      `SELECT id FROM spaces WHERE is_dm = 1 AND name LIKE ? LIMIT 1`,
      [`self_${myUserId}_%`],
      (err, row) => {
        if (err) return res.status(500).json({ error: "Database error" });
        ...
        db.run(
          `INSERT INTO spaces (name, created_by, is_private, is_dm) VALUES (?, ?, 1, 1)`,
          [`self_${myUserId}_${Date.now()}`, req.user.username],
          function (err) {
            if (err)
    ...
    `,
      ...
      (err, row) => {
        ...
        db.run(
          `INSERT INTO spaces (name, created_by, is_private, is_dm) VALUES (?, ?, 1, 1)`,
          [`dm_${myUserId}_${targetUserId}_${Date.now()}`, req.user.username],
          function (err) {
            if (err)
              return res.status(500).json({ error: "Failed to create DM" });
            const newId = this.lastID;
                ...
                db.run(
                  ...
                  () => {
                    insertKeyShares(newId, () => {
                      io.emit("space created", {
                        id: newId,
                        name: `dm_${myUserId}_${targetUserId}`,
                        created_by: req.user.username,
                        is_private: 1,
                        is_dm: 1,
                      });
      ...
      (err, rows) => {
        ...
        res.json(
          rows
            ...
            .map((r) => ({
              ...r,
              timestamp: r.timestamp ? r.timestamp + "Z" : null,
            })),
        );
      },
    );
  ...
  db.get(
    ...
    (err, space) => {
      ...
      db.run(
        ...
        [spaceId, parseInt(userId, 10)],
        function (err) {
          if (err) return res.status(500).json({ error: "Database error" });
          if (this.changes === 0) {
            console.error(
              "REMOVE ERROR: No rows deleted for spaceId=",
              spaceId,
              "userId=",
              userId,
            );
          ...
          db.run(
            "DELETE FROM space_keys WHERE space_id = ? AND user_id = ?",
            [spaceId, parseInt(userId, 10)],
            () => {},
          );
...
app.post("/api/spaces/:id/leave", authenticateToken, (req, res) => {
  ...
  if (spaceId === 1)
    return res.status(403).json({ error: "Cannot leave the General space" });
  ...
  db.run(
    ...
    function (err) {
      ...
      if (this.changes === 0) {
        console.error(
          "LEAVE ERROR: No rows deleted. spaceId=",
          spaceId,
          "uId=",
          uId,
          "req.user=",
...
app.delete("/api/spaces/:id", authenticateToken, (req, res) => {
  ...
  db.get(
    "SELECT created_by, name, is_dm FROM spaces WHERE id = ?",
    [spaceId],
    (err, space) => {
      if (err || !space)
        return res.status(404).json({ error: "Space not found" });
      ...
      if (space.is_dm === 1 && space.name.startsWith("self_")) {
        return res
          .status(403)
          .json({ error: "Cannot delete your Notes to Self space" });
      }

      if (req.user.role !== "admin" && req.user.username !== space.created_by) {
        return res
          .status(403)
          .json({ error: "Only admins or the creator can delete this space." });
      }
...
app.post(
  ...
  async (req, res) => {
    ...
    if (maxMb > 0 && req.file.size > maxMb * 1024 * 1024) {
      ...
      return res
        .status(413)
        .json({ error: `File exceeds maximum upload size of ${maxMb}MB` });
    }
    ...
    if (ext === "heic" || type === "image/heic" || type === "image/heif") {
      try {
        console.log(
          `HEIC image detected via HTTP upload, transcoding to JPEG...`,
        );
        ...
        console.error(
          "Backend HEIC transcoding failed (falling back to original):",
          err,
        );
      }
    }

    // Backend Video Transcoding (HEVC + 3GP) — skip voice messages
    const is3gp = ext === "3gp" || type === "video/3gpp";
    const isVoice = origName.startsWith("voice-") || type.startsWith("audio/");
    if ((type.startsWith("video/") || is3gp) && !isVoice) {
      let isHevc = false;
      if (!is3gp) {
        isHevc = await new Promise((resolve) => {
          ffmpeg.ffprobe(req.file.path, (err, metadata) => {
            ...
            resolve(
              stream &&
                (stream.codec_name === "hevc" || stream.codec_name === "h265"),
            );
          });
        }).catch(() => false);
      }
      ...
      if (isHevc || is3gp) {
        console.log(
          `Unplayable video detected HTTP upload (${is3gp ? "3GP" : "HEVC"}), transcoding to H.264...`,
        );
...
io.on("connection", (socket) => {
  ...
  db.get(
    "SELECT username, first_name, last_name, avatar FROM users WHERE username = ?",
    [username],
    (err, row) => {
      if (!err && row) {
        ...
        io.emit(
          "presence",
          Array.from(activeUsers.values()).map((v) => v.user),
        );
      }
    },
  );
  ...
  db.all(
    `SELECT pkr.space_id, pkr.user_id, u.public_key 
          FROM pending_key_requests pkr 
          JOIN users u ON u.id = pkr.user_id 
          JOIN spaces s ON s.id = pkr.space_id 
          WHERE s.escrow_key IS NOT NULL AND u.public_key IS NOT NULL`,
    [],
    (err, pending) => {
        ...
        try {
          ...
          for (const [, s] of io.sockets.sockets) {
            if (s.user && s.user.userId === req.user_id) {
              s.emit("grant_room_key", {
                spaceId: req.space_id,
                encryptedRoomKey: wrappedB64,
              });
            }
          }
          console.log(
            `[E2EE] Fulfilled pending key request: space ${req.space_id} -> user ${req.user_id}`,
          );
        } catch (e) {
          console.error(
            `[E2EE] Failed to fulfill pending key for space ${req.space_id}`,
            e,
          );
        }
      });
  ...
  db.all(
    "SELECT id FROM spaces WHERE is_private = 0",
    [],
    (err, publicRows) => {
      if (!err && publicRows)
        publicRows.forEach((r) => socket.join(r.id.toString()));
    },
  );
  ...
  db.all(
    ...
    (err, privateRows) => {
      if (!err && privateRows)
        privateRows.forEach((r) => socket.join(r.space_id.toString()));
    },
  );
  ...
  socket.on("stop typing", (data) => {
    ...
    socket
      .to(spaceId.toString())
      .emit("user stopped typing", { username, spaceId });
  });
      ...
      const proceedWithJoin = () => {
        ...
        `,
          ...
          (err, rows) => {
            ...
            db.all(
              "SELECT username, message_id FROM read_receipts WHERE space_id = ?",
              [spaceId],
              (err, receipts) => {
                if (!err) {
                  const receiptMap = {};
                  receipts.forEach(
                    (r) => (receiptMap[r.username] = r.message_id),
                  );
  ...
  socket.on("grant_room_key", (data) => {
    ...
    db.run(
      "INSERT OR REPLACE INTO space_keys (space_id, user_id, encrypted_room_key) VALUES (?, ?, ?)",
      [spaceId, requesterId, encryptedRoomKey],
      (err) => {
    ...
    if (requesterSocketId) {
      io.to(requesterSocketId).emit("grant_room_key", {
        spaceId,
        encryptedRoomKey,
      });
    }
  ...
  socket.on("chat message", async (msg) => {
    ...
    db.get(
      "SELECT avatar, first_name, last_name FROM users WHERE username = ?",
      [sender],
      (err, user) => {
            ...
            if (!err) {
              ...
              db.get(
                ...
                (err, space) => {
                  ...
                  // Walk full reply chain to find the true root message for thread-aware notifications
                  const rootLookup = replyTo
                    ? new Promise((resolve) => {
                        const walkChain = (id, visited = new Set()) => {
                          if (visited.has(id)) return resolve(id);
                          visited.add(id);
                          db.get(
                            "SELECT id, reply_to, sender FROM messages WHERE id = ?",
                            [id],
                            (err, row) => {
                              if (err || !row || !row.reply_to)
                                return resolve(id);
                              walkChain(row.reply_to, visited);
                            },
                          );
                        };
                        walkChain(replyTo);
                  ...
                  const parentSenderLookup = replyTo
                    ? new Promise((resolve) => {
                        db.get(
                          "SELECT sender FROM messages WHERE id = ?",
                          [replyTo],
                          (err, row) => {
                            resolve(row ? row.sender : null);
                          },
                        );
                      })
                    : Promise.resolve(null);

                  // Look up all thread participants by finding the root first, then collecting all replies
                  const threadParticipantsLookup = replyTo
                    ? rootLookup.then((rootId) => {
                        if (!rootId) return [];
                        return new Promise((resolve) => {
                          // Recursive CTE to find ALL messages in the thread tree
                          db.all(
                            `WITH RECURSIVE thread_tree(id) AS (
                          SELECT id FROM messages WHERE id = ?
                          UNION ALL
                          SELECT m.id FROM messages m JOIN thread_tree t ON m.reply_to = t.id
                        )
                        SELECT DISTINCT sender FROM messages WHERE id IN (SELECT id FROM thread_tree)`,
                            [rootId],
                            (err, rows) => {
                              resolve(rows ? rows.map((r) => r.sender) : []);
                            },
                          );
                        });
                      })
                  ...
                  parentSenderLookup.then((parentSender) => {
                    threadParticipantsLookup.then((threadParticipants) => {
                      // Only notify members of this space who aren't the sender
                      db.all(
                        `SELECT u.id, u.username FROM users u 
                        JOIN space_members sm ON u.id = sm.user_id 
                        WHERE sm.space_id = ? AND u.username != ?`,
                        [spaceId, sender],
                        (err, members) => {
                          if (!err && members) {
                            members.forEach((m) => {
                              // Build context-aware title per recipient
                              let title;
                              if (replyTo && parentSender) {
                                if (m.username === parentSender) {
                                  // Recipient is the root message author
                                  title = isDm
                                    ? `${displayName} replied to your message`
                                    : `${displayName} replied to your message in #${spaceName}`;
                                } else if (
                                  threadParticipants.includes(m.username)
                                ) {
                                  // Recipient is a thread participant (replied previously)
                                  title = isDm
                                    ? `${displayName} replied to a thread you're in`
                                    : `${displayName} replied to a thread you're in #${spaceName}`;
                                } else {
                                  // Recipient is NOT a thread participant
                                  title = isDm
                                    ? `${displayName} replied in a thread`
                                    : `${displayName} replied in a thread in #${spaceName}`;
                                }
                              } else {
                                // Not a threaded reply — default behavior
                                if (isDm) {
                                  title = displayName;
                                } else {
                              ...
                              const pushPayload = {
                                ...
                                icon:
                                  outgoingMsg.avatar &&
                                  !outgoingMsg.avatar.startsWith("data:")
                                    ? outgoingMsg.avatar
                                    : "/icon.png",
                                data: {
                                  spaceId,
                                  ...
                                  isThreadReply: !!replyTo,
                                  parentMessageSender: parentSender,
                                  timestamp: Date.now(),
                              ...
                              sendPushNotification(m.id, pushPayload);
                            });
                          }
                        },
                      );
  ...
  socket.on("edit message", (data) => {
    ...
    db.get("SELECT sender FROM messages WHERE id = ?", [id], (err, row) => {
      ...
      db.run(
        ...
        (err) => {
          if (!err) {
            io.to(spaceId.toString()).emit("message updated", {
              id,
              text,
              edited: 1,
              spaceId,
  ...
  socket.on("mention", (data) => {
    const { spaceId, mentionedUsers, sender } = data;
    if (!spaceId || !mentionedUsers || !Array.isArray(mentionedUsers)) return;
    ...
    db.get(
      ...
      (err, space) => {
        ...
        mentionedUsers.forEach((mentionedUsername) => {
          ...
          // Send push notification for mentioned user
          db.get(
  ...
  socket.on("delete message", (data) => {
    ...
    db.get(
      "SELECT m.sender, m.asset, s.created_by FROM messages m JOIN spaces s ON m.space_id = s.id WHERE m.id = ?",
      [id],
      (err, row) => {
        if (
          err ||
  ...
  socket.on("pin message", (data) => {
    ...
    const userRole = socket.user.role;
    db.get(
      "SELECT m.sender, s.created_by FROM messages m JOIN spaces s ON m.space_id = s.id WHERE m.id = ?",
      [id],
      (err, row) => {
        if (err || !row || (row.created_by !== sender && userRole !== "admin"))
          return;
        db.run(
          ...
          (err) => {
            if (!err)
              io.to(spaceId.toString()).emit("message pinned", {
                id,
  ...
  socket.on("react message", (data) => {
    ...
    db.get("SELECT reactions FROM messages WHERE id = ?", [id], (err, row) => {
      ...
      db.run(
        ...
        (err) => {
          if (!err)
            io.to(spaceId.toString()).emit("message reacted", {
              id,
  ...
  socket.on("call-invite", ({ spaceId, targetUserIds, audioOnly }) => {
    ...
    for (const [socketId, s] of io.of("/").sockets) {
      if (
        s.user &&
        targetUserIds.includes(s.user.userId) &&
        socketId !== socket.id
      ) {
    ...
    if (targetUserIds.length > 0) {
      ...
      db.get(
        ...
        (err, space) => {
          console.log(
            `[CALL] ${caller.username} invited ${targetUserIds.length} user(s) to ${audioOnly ? "audio" : "video"} call in space ${space?.name || spaceId}`,
          );
        },
      );
    }
  ...
  socket.on("admin broadcast", (data) => {
    ...
    db.get(
      "SELECT role, first_name, last_name FROM users WHERE username = ?",
      [username],
      (err, row) => {
